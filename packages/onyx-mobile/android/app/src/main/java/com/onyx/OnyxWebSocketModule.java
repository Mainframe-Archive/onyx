...
@ReactModule(name = "OnyxWebSocketModule", hasConstants = false)
public final class OnyxWebSocketModule extends ReactContextBaseJavaModule {
    ...
    public OnyxWebSocketModule(ReactApplicationContext context) {
        super(context);
    ...
    public void connect(
            ...
            final int id,
            @Nullable final String clientCertPath,
            @Nullable final String certPassword,
            @Nullable final String caCertPath
            ) {

        OkHttpClient.Builder clientBuilder = new OkHttpClient.Builder();

        if (clientCertPath != null && caCertPath != null) {
            try {
                KeyStore keyStore = KeyStore.getInstance("PKCS12");

                // Import Client Cert
                InputStream clientCertInputStream = new FileInputStream(clientCertPath);
                keyStore.load(clientCertInputStream, certPassword.toCharArray());
                ...
                FLog.w(ReactConstants.TAG, "Cert load error: " + e.toString());
            }
        }
