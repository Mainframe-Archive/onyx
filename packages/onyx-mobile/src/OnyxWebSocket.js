...
const WebSocketEvent = require('./OnyxWebSocketEvent');
 ...
 */
class OnyxWebSocket extends EventTarget(...WEBSOCKET_EVENTS) {
  ...
  static isAvailable: boolean = !!OnyxWebSocketModule;
  ...
  send(data: string | ArrayBuffer | ArrayBufferView | Blob): void {
    ...
    if (typeof data === 'string') {
      OnyxWebSocketModule.send(data, this._socketId);
      return;
    }

    if (data instanceof ArrayBuffer || ArrayBuffer.isView(data)) {
      OnyxWebSocketModule.sendBinary(binaryToBase64(data), this._socketId);
      return;
    }
  ...
  ping(): void {
    ...
    OnyxWebSocketModule.ping(this._socketId);
  }
  ...
  _close(code?: number, reason?: string): void {
    if (Platform.OS === 'android') {
      ...
      OnyxWebSocketModule.close(statusCode, closeReason, this._socketId);
    } else {
      OnyxWebSocketModule.close(this._socketId);
    }
  }
