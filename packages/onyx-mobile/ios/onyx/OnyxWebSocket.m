...
@interface NSData (OnyxWebSocket)
...
@interface NSString (OnyxWebSocket)
...
@interface _OnyxRunLoopThread : NSThread
...
@implementation NSData (OnyxWebSocket)
...
@implementation NSString (OnyxWebSocket)
...
NSString *const OnyxWebSocketErrorDomain = @"OnyxWebSocketErrorDomain";
NSString *const OnyxHTTPResponseErrorKey = @"HTTPResponseStatusCode";

// Returns number of bytes consumed. Returning 0 means you didn't match.
// Sends bytes to callback handler;
typedef size_t (^stream_scanner)(NSData *collected_data);

typedef void (^data_callback)(OnyxWebSocket *webSocket,  NSData *data);

@interface OnyxIOConsumer : NSObject
...
@interface OnyxIOConsumerPool : NSObject

- (instancetype)initWithBufferCapacity:(NSUInteger)poolSize NS_DESIGNATED_INITIALIZER;

- (OnyxIOConsumer *)consumerWithScanner:(stream_scanner)scanner handler:(data_callback)handler bytesNeeded:(size_t)bytesNeeded readToCurrentFrame:(BOOL)readToCurrentFrame unmaskBytes:(BOOL)unmaskBytes;
- (void)returnConsumer:(OnyxIOConsumer *)consumer;

@end

@interface OnyxWebSocket ()  <NSStreamDelegate>
...
@implementation OnyxWebSocket
{
  ...
  NSMutableArray<OnyxIOConsumer *> *_consumers;
  ...
  __strong OnyxWebSocket *_selfRetain;
  
  NSArray<NSString *> *_requestedProtocols;
  OnyxIOConsumerPool *_consumerPool;
...
- (void)_RCTSR_commonInit;
{
  ...
  _consumerPool = [OnyxIOConsumerPool new];
...
{
  assert(_url);
  RCTAssert(_readyState == RCTSR_CONNECTING, @"Cannot call -(void)open on OnyxWebSocket more than once");
...
- (void)_HTTPHeadersDidFinish;
{
  ...
  if (responseCode >= 400) {
    ...
    [self _failWithError:[NSError errorWithDomain:OnyxWebSocketErrorDomain code:2132 userInfo:@{NSLocalizedDescriptionKey:[NSString stringWithFormat:@"received bad response code from server %ld", (long)responseCode], OnyxHTTPResponseErrorKey:@(responseCode)}]];
    return;
  }
  
  if (![self _checkHandshake:_receivedHTTPHeaders]) {
    [self _failWithError:[NSError errorWithDomain:OnyxWebSocketErrorDomain code:2133 userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Invalid Sec-WebSocket-Accept response"]}]];
    return;
  }
  ...
  if (negotiatedProtocol) {
    ...
    if ([_requestedProtocols indexOfObject:negotiatedProtocol] == NSNotFound) {
      [self _failWithError:[NSError errorWithDomain:OnyxWebSocketErrorDomain code:2133 userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Server specified Sec-WebSocket-Protocol that wasn't requested"]}]];
      return;
    }
...
- (void)_readHTTPHeader;
{
  ...
  [self _readUntilHeaderCompleteWithCallback:^(OnyxWebSocket *socket,  NSData *data) {
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData;
{
  ...
  if (frame_header.payload_length == 0) {
    ...
    [self _addConsumerWithDataLength:(size_t)frame_header.payload_length callback:^(OnyxWebSocket *socket, NSData *newData) {
      if (isControlFrame) {
...
- (void)_readFrameContinue;
{
  ...
  [self _addConsumerWithDataLength:2 callback:^(OnyxWebSocket *socket, NSData *data) {
    ...
    if (extra_bytes_needed == 0) {
      ...
      [socket _addConsumerWithDataLength:extra_bytes_needed callback:^(OnyxWebSocket *_socket, NSData *_data) {
...
- (void)_pumpWriting;
{
  ...
  if (dataLength - _outputBufferOffset > 0 && _outputStream.hasSpaceAvailable) {
    ...
    if (bytesWritten == -1) {
      [self _failWithError:[NSError errorWithDomain:OnyxWebSocketErrorDomain code:2145 userInfo:@{NSLocalizedDescriptionKey: @"Error writing to stream"}]];
      return;
    }
...
- (BOOL)_innerPumpScanner
{
  ...
  OnyxIOConsumer *consumer = _consumers[0];
...
- (void)stream:(NSStream *)aStream handleEvent:(NSStreamEvent)eventCode;
{
  if (_secure && !_pinnedCertFound && (eventCode == NSStreamEventHasBytesAvailable || eventCode == NSStreamEventHasSpaceAvailable)) {
    ...
    if (sslCerts) {
      ...
      if (!_pinnedCertFound) {
        dispatch_async(_workQueue, ^{
          [self _failWithError:[NSError errorWithDomain:OnyxWebSocketErrorDomain code:23556 userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"Invalid server cert"]}]];
        });
        return;
      }
    }
...
@implementation OnyxIOConsumerPool
{
  NSUInteger _poolSize;
  NSMutableArray<OnyxIOConsumer *> *_bufferedConsumers;
}
...
- (OnyxIOConsumer *)consumerWithScanner:(stream_scanner)scanner handler:(data_callback)handler bytesNeeded:(size_t)bytesNeeded readToCurrentFrame:(BOOL)readToCurrentFrame unmaskBytes:(BOOL)unmaskBytes;
{
  OnyxIOConsumer *consumer = nil;
  if (_bufferedConsumers.count) {
    consumer = _bufferedConsumers.lastObject;
    [_bufferedConsumers removeLastObject];
  } else {
    consumer = [OnyxIOConsumer new];
  }
...
@implementation NSURL (OnyxWebSocket)
...
static _OnyxRunLoopThread *networkThread = nil;
static NSRunLoop *networkRunLoop = nil;

@implementation NSRunLoop (OnyxWebSocket)

+ (NSRunLoop *)RCTSR_networkRunLoop
{
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    networkThread = [_OnyxRunLoopThread new];
...
@implementation _OnyxRunLoopThread
{
